...
def ResNet50(include_top=True, weights='imagenet',
    ...
    x = ZeroPadding2D(padding=(3, 3), name=name+'conv1_pad')(img_input)
    x = Conv2D(64, (7, 7), strides=(2, 2), padding='valid', name=name+'conv1')(x)
    x = BatchNormalization(axis=bn_axis, name=name+'bn_conv1')(x)
    x = Activation('relu', name=name+'relu')(x)
    x = MaxPooling2D((3, 3), strides=(2, 2), name=name+'maxpooling2d_1')(x)

    x = conv_block(x, 3, [64, 64, 256], stage=2, block=name+'a', strides=(1, 1))
    ...
    x = AveragePooling2D((7, 7), name=name+'avg_pool')(x)
